...
def test_func():
    ...
    model = CloneModel(50, 200, ).to(device)

    node_1 = torch.tensor(np.random.randint(0, 10, [10]), dtype=torch.long)
    g_1 = np.ones([10, 10])
    node_2 = torch.tensor(np.random.randint(0, 20, [5]), dtype=torch.long)
    g_2 = np.ones([5, 5])
    node_list_1 = [node_1, node_2]
    g_list_1 = [g_1, g_2]

    model(node_list_1, node_list_1, g_list_1, g_list_1, device)
