...
class BOWEncoder(nn.Module):
    ...
    '''
    def __init__(self, vocab_size, emb_size, hidden_size, embed_tensor):
        super(BOWEncoder, self).__init__()
        self.emb_size=emb_size
        self.hidden_size = hidden_size

        if torch.is_tensor(embed_tensor):
            vocab_size = len(embed_tensor)
            self.embedding = nn.Embedding(vocab_size, emb_size, _weight=embed_tensor)
...
class SeqEncoder(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size, embed_tensor, embed_type, n_layers=1):
        ...
        if torch.is_tensor(embed_tensor):
            vocab_size = len(embed_tensor)
            if embed_type == 0:
                self.embedding = nn.Embedding(vocab_size, emb_size, padding_idx=1, _weight=embed_tensor)
            else:
                self.embedding = nn.Embedding(vocab_size, emb_size, padding_idx=0, _weight=embed_tensor)
            self.embedding.weight.requires_grad = False
        else:
