...
def build_dict(dataset):
    ...
    with open('../data/java-small-preprocess/tk.pkl', 'wb') as f:
...
def norm_data(data_type):
    file_name = '../data/java-small/java-small.' + data_type + '.c2s'
    with open(file_name, 'r') as f:
        dataset = f.readlines()
    with open('../data/java-small-preprocess/tk.pkl', 'rb') as f:
    ...
    save_file = '../data/java-small-preprocess/' + data_type + '.pkl'
...
def main():
    with open('../data/java-small/java-small.train.c2s', 'r') as f:
        dataset = f.readlines()
        print('data number is ', len(dataset))
    if not os.path.isdir('../data/java-small-preprocess'):
        os.mkdir('../data/java-small-preprocess')
    build_dict(dataset)
    norm_data('train')
    norm_data('val')
    norm_data('test')
